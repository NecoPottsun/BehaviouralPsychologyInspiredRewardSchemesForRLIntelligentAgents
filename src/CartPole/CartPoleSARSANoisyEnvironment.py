...
class CartPoleSARSANoisyEnvironment:
    ...
    def train(self, episodes, render):
        ...
        path = 'cart_pole_SARSA_' + self.environment_name
        q_path = path + '.pkl'
        f = open(q_path, 'wb')
        ...
        plt.suptitle('Cart pole SARSA, Noisy = %s, Time = %f' % (self.environment_name,
                     end_time_running_all_episodes))
        png_file = auxFunctions.create_filename(
            'cart_pole', 'SARSA', 'png', other=self.environment_name)
