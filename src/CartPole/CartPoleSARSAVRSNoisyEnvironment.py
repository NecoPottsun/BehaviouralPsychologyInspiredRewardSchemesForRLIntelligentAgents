...
class CartPoleSARSAVRSNoisyEnvironment:
    ...
    def train(self, episodes, render):
        ...
        for episode in range(episodes):
            ...
            for t in range(MAX_TRAIN_T):
                ...
                q_table[state + (action,)] += learning_rate*(reward +
                                                             discount_factor*(best_q) + bc - q_table[state + (action,)])
        ...
        path = 'cart_pole_SARSA_VRS_' + self.reward_schedule + '_' + self.environment_name
        q_path = path + '.pkl'
        f = open(q_path, 'wb')
        ...
        plt.suptitle('Cart pole SARSA applying VRS %s, Noisy = %s, Time = %f' % (self.reward_schedule, self.environment_name,
                     end_time_running_all_episodes))
        png_file = auxFunctions.create_filename(
            'cart_pole', 'SARSA', 'png', other=other)
