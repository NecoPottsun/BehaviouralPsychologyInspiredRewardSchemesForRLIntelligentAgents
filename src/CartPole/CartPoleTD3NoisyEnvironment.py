...
class CartPoleTD3NoisyEnvironment:
    ...
    def train(self, episodes, render):
        ...
        for episode in range(episodes):
            ...
            for t in range(MAX_TRAIN_T):
                ...
                if terminated or truncated:
                    td_target_estimate = reward + discount_factor * (best_q)
                else:
                ...
                q_table[state + (action,)] += learning_rate * \
                    (td_target_estimate - q_table[state + (action,)])
        ...
        path = 'cart_pole_TD3_' + self.environment_name
        q_path = path + '.pkl'
        f = open(q_path, 'wb')
        ...
        plt.suptitle('Cart pole TD(3), Noisy = %s, Time = %f' % (self.environment_name,
                     end_time_running_all_episodes))
        png_file = auxFunctions.create_filename(
            'cart_pole', 'TD3', 'png', other=self.environment_name)
