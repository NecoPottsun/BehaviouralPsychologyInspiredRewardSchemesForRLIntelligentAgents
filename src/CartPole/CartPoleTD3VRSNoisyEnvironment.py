...
- Implement 3-step Expected SARSA algorithm
...
class CartPoleTD3VRSNoisyEnvironment:
    ...
    def train(self, episodes, render):
        ...
        for episode in range(episodes):
            ...
            for t in range(MAX_TRAIN_T):
                ...
                # Take action for the next_state -> next_action (2)
                next_action = self.select_action(
                    next_state, explore_rate, q_table)

                best_q = q_table[next_state + (next_action, )]
                # Re-calculate reward using 1 of the 4 variable reward schemes
                reward *= auxFunctions.select_reward_schedule(
                    self.reward_schedule, time_step=t, last_reinforcement_time=last_reinforcement_time)
                ...
                if terminated or truncated:
                    td_target_estimate = reward + discount_factor * \
                        q_table[next_state + (next_action,)]
                else:
                    # Observe the next state of next_state -> next_next_state
                    next_next_state, next_reward, next_terminated, next_truncated, _ = env.step(
                        next_action)
                    # Re-calculate next_reward using 1 of the 4 variable reward schedules
                    next_reward *= auxFunctions.select_reward_schedule(
                        self.reward_schedule, time_step=t, last_reinforcement_time=last_reinforcement_time)
                    next_next_state = self.state_to_bucket(next_next_state)
                    # Take action for the next_next_state -> next_next_action (3)
                    next_next_action = self.select_action(
                        next_next_state, explore_rate, q_table)

                    if next_terminated or next_truncated:
                        td_target_estimate = reward + discount_factor*next_reward + \
                            (discount_factor**2) * \
                            q_table[next_next_state + (next_next_action,)]
                    else:
                        # Observe the next state of next_next_state -> next_next_next_state
                        next_next_next_state, next_next_reward, next_next_terminated, next_next_truncated, _ = env.step(
                            next_next_action)
                        # Re-calculate next_next_reward using 1 of the 4 variable reward schedules
                        next_next_reward *= auxFunctions.select_reward_schedule(
                            self.reward_schedule, time_step=t, last_reinforcement_time=last_reinforcement_time)
                        next_next_next_state = self.state_to_bucket(
                            next_next_next_state)
                        # Take action for the next_next_next_state -> next_next_next_action (4)
                        next_next_next_action = self.select_action(
                            next_next_next_state, explore_rate, q_table)
                        td_target_estimate = reward + discount_factor*next_reward + (discount_factor**2)*next_next_reward + (
                            discount_factor**3)*q_table[next_next_next_state + (next_next_next_action, )]
                # Update the Q based on the result
                q_table[state + (action,)] += learning_rate * \
                    (td_target_estimate + bc - q_table[state + (action, )])
        ...
        path = 'cart_pole_TD3_VRS_' + self.reward_schedule + '_' + self.environment_name
        q_path = path + '.pkl'
        f = open(q_path, 'wb')
        ...
        plt.suptitle('Cart pole TD(3) applying VRS %s, Noisy = %s, Time = %f' % (self.reward_schedule, self.environment_name,
                     end_time_running_all_episodes))
        png_file = auxFunctions.create_filename(
            'cart_pole', 'TD3', 'png', other=other)
