...
class MountainCarQLEARNINGNoisyEnvironment:
    ...
    def run(self, episodes, is_training, render):
        ...
        # Initialize path
        path = 'mountain_car_Q-Learning_' + self.environment_name
        q_path = path + '.pkl'
        ...
        if is_training:
            ...
            plt.suptitle('Mountain car Q-Learning, Noisy = %s,Time = %f' % (self.environment_name,
                         end_time_running_all_episodes))
            png_file = auxFunctions.create_filename(
                'mountain_car', 'Q-Learning', 'png', other=self.environment_name)
