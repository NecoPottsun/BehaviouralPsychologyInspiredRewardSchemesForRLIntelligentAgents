...
class MountainCarTD3NoisyEnvironment:
    ...
    def run(self, episodes, is_training, render):
        ...
        # Initialize path
        path = 'mountain_car_TD3_' + self.environment_name
        q_path = path + '.pkl'
            ...
            while (not terminated and step <= STEPS):
                ...
                if is_training:
                    ...
                    if next_state_position >= env.goal_position:
                        td_target_estimate = reward + DISCOUNT_RATE * \
                    ...
                    Q[state][action] += LEARNING_RATE * \
                        (td_target_estimate - Q[state][action])
                if next_state_position >= env.goal_position:
        ...
        if is_training:
            ...
            plt.suptitle('Mountain car TD(3), Noisy = %s, Time = %f' % (self.environment_name,
                         end_time_running_all_episodes))
            png_file = auxFunctions.create_filename(
                'mountain_car', 'TD3', 'png', other=self.environment_name)
