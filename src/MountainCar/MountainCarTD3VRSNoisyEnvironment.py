...
class MountainCarTD3VRSNoisyEnvironment:
    ...
    def run(self, episodes, is_training, render):
        ...
        # Initialize path
        path = 'mountain_car_TD3_VRS_' + self.reward_schedule + '_' + self.environment_name
        q_path = path + '.pkl'
        ...
        for i in range(episodes):
            ...
            while (not terminated and step <= STEPS):
                ...
                if is_training:
                    # Check if next_state is the goal
                    if next_state_position >= env.goal_position:
                        td_target_estimate = reward + \
                            DISCOUNT_RATE*Q[next_state][next_action]
                    else:
                        # Observe the next state of next_state -> next_next_state
                        next_next_state, next_reward, next_terminated, _, _ = env.step(
                            next_action)
                        # Re-calculate next_reward using 1 of the 4 variable reward schedules
                        next_reward *= auxFunctions.select_reward_schedule(
                            self.reward_schedule, time_step=step, last_reinforcement_time=last_reinforcement_time)
                        next_next_state_position = next_next_state[0]
                        next_next_state = self.discretize_state(
                            next_next_state)
                        # Take action for the next_next_state -> next_next_action (3)
                        next_next_action = self.epsilon_greedy_policy(
                            is_training, epsilon, Q, next_next_state)
                        if next_next_state_position >= env.goal_position:
                            td_target_estimate = reward + DISCOUNT_RATE * next_reward + \
                                (DISCOUNT_RATE**2) * \
                                Q[next_next_state][next_next_action]
                        else:
                            # Observe the next state of next_next_state -> next_next_next_state
                            next_next_next_state, next_next_reward, next_next_terminated, _, _ = env.step(
                                next_next_action)
                            next_next_next_state = self.discretize_state(
                            ...
                            next_next_next_action = self.epsilon_greedy_policy(
                                is_training, epsilon, Q, next_next_next_state)
                            # Calculate the td target estimate
                            td_target_estimate = reward + DISCOUNT_RATE*next_reward + \
                                (DISCOUNT_RATE**2) * next_next_reward + (DISCOUNT_RATE **
                                                                         3) * Q[next_next_next_state][next_next_next_action]
                    # Update Q value
                    Q[state][action] += LEARNING_RATE * \
                        (td_target_estimate + bc - Q[state][action])
                if next_state_position >= env.goal_position:
        ...
        if is_training:
            ...
            plt.suptitle('Mountain car TD(3) applying VRS %s, Noisy = %s, Time = %f' % (self.reward_schedule, self.environment_name,
                         end_time_running_all_episodes))
            png_file = auxFunctions.create_filename(
                'mountain_car', 'TD3', 'png', other=other)
