# Adapted from noisyenv package
# UCL Institute of Finance & Technology (https://github.com/UCL-IFT/noisyenv)
...
class RandomMixupObservation(gym.ObservationWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomMixupObservation
    ...
    def observation(self, observation):
        ...
        if (np.random.rand() <= self.noise_rate) and (self._last_observation is not None):
            observation = self.factor * observation + \
                (1 - self.factor) * self._last_observation
...
class RandomDropoutObservation(gym.ObservationWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomDropoutObservation
        >>> env = gym.make("CartPole-v1")
    ...
    def observation(self, observation):
        ...
        if (np.random.rand() <= self.noise_rate):
            observation *= np.random.binomial(
                np.ones(observation.shape).astype(int), p=(1 - self.p))
        return observation
...
class RandomNormalNoisyObservation(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        """Returns the potentially modified observation."""
        if np.random.rand() <= self.noise_rate:
            observation += np.random.normal(loc=self.loc,
                                            scale=self.scale, size=observation.shape)
        return observation
...
class RandomUniformNoisyObservation(gym.ObservationWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomUniformNoisyObservation
        >>> env = gym.make("CartPole-v1")
    ...
    def observation(self, observation):
        """Returns the potentially modified observation."""
        if np.random.rand() <= self.noise_rate:
            observation += np.random.uniform(low=self.low,
                                             high=self.high, size=observation.shape)
        return observation
...
class RandomUniformScaleObservation(gym.ObservationWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomUniformScaleObservation
        >>> env = gym.make("CartPole-v1")
...
class RandomUniformScaleReward(gym.RewardWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomUniformScaleReward
        >>> env = gym.make("CartPole-v1")
...
class RandomUniformNoisyReward(gym.RewardWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomUniformNoisyReward
        >>> env = gym.make("CartPole-v1")
...
class RandomNormalNoisyReward(gym.RewardWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from NoisyEnvironment import RandomNormalNoisyReward
        >>> env = gym.make("CartPole-v1")
